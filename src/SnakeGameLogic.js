...
SnakeGameLogic.prototype.nextState = function () {
  ...
  if (
     newHead.y >= ROW || newHead.y < 0 || newHead.x >= COLS || newHead.x < 0 || this.joints.some(joint =>joint.x === newHead.x && joint.y === newHead.y)
     {
      return false;
     }
     ...
     if (newHead.x === this.fruit.x && newHead.y === this.fruit.y){
        do {
          ...
          (newFruit.x === newHead.x && newFruit.y === newHead.y) || this.joints.some
